...
def export():
    ...
    try:
        ...
        export_dir = "./export/lstm_v1/"
        os.makedirs(export_dir, exist_ok=True)
        print(f"Directory {export_dir} verified.")
        
        # Load the processed results
        if not os.path.exists('model_results_q1_2026.csv'):
            print("Error: model_results_q1_2026.csv not found.")
            return

        res_df = pd.read_csv('model_results_q1_2026.csv')
    ...
    try:
        ...
        model.save_model("model_xgb_2026.json")
        print("model_xgb_2026.json saved successfully.")
    except Exception as e:
    ...
    try:
        ...
        with open(os.path.join(export_dir, 'sequential_component_info.txt'), 'w') as f:
