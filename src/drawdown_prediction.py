...
if __name__ == "__main__":
    ...
    if not os.path.exists('german_gas_q1_2026.csv'):
        print("Error: Energy data CSV not found.")
        sys.exit(1)
        
    gas_data = pd.read_csv('german_gas_q1_2026.csv', parse_dates=['Date']).set_index('Date')
    ...
    try:
        ...
        ds_t = xr.open_dataset('era5_land_2026_jan_feb_REAL.grib', engine='cfgrib', 
                              chunks={'time': 12},
                              backend_kwargs={'filter_by_keys': {'shortName': '2t'}, 'indexpath': idx_t})
        
        ds_p = xr.open_dataset('era5_land_2026_jan_feb_REAL.grib', engine='cfgrib', 
                              chunks={'time': 12},
                              backend_kwargs={'filter_by_keys': {'shortName': 'tp'}, 'indexpath': idx_p})
        
        ds_t = ds_t.sel(**germany_box)
        ds_p = ds_p.sel(**germany_box)
        ds_clime = xr.open_dataset('era5_climatology_1991_2020_REAL.nc')
        ...
        results.to_csv('model_results_q1_2026.csv')
